...
library(leaflet)
m <- data.table(rbind(c("Boston", "Patriots", "AFC", "Gillette Stadium", 1, 42.0909, -71.2643), 
...
leaflet(m) %>% addTiles() %>% setView(lat=40, lng=-98, zoom=4) %>% addCircleMarkers(lng= ~Long, lat= ~Lat)
